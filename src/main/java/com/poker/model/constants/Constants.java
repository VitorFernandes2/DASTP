package com.poker.model.constants;

public class Constants {
    public static final Integer MAXIMUM_NUMBER_CARD = 13;
    public static final Integer MINIMUM_NUMBER_CARD = 1;
    public static final Integer MINIMUM_PCJ_BET = 4;
    public static final Integer MINIMUM_GAME_PLAYERS = 2;
    public static final Integer MAXIMUM_FRIENDLY_GAME_PLAYERS = 3;
    public static final Integer MONEY_CONVERSION_RATE = 100;
    public static final Integer PC_CONVERSION_RATE = 50;
    public static final Double PAYPAL_INTEREST_RATE = 0.05;
    public static final Integer FRIENDLY_GAME_MINIMUM_PLAYERS = 3;

    // Commands
    public static final String COMMAND = "command";
    public static final String COMMAND_LAST_DIVISION = "lastDivision";
}
